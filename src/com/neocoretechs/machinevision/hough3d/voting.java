...
public class voting {
	 ...
	 * @return
	 */
boolean cast_vote(ArrayList<bin_t> used_bins, accum_cell_t cell, kernel_t kernel, double votes, double[] tpr/*double t, int p, int r*/){
...
boolean gaussian_vote_1d(accumulatorball_t accum, kernel_t kernel, ArrayList<bin_t> used_bins, 
   ...
   //p = phi_index;
   //t = theta_index;
   double[] theta_phi_index = new double[]{theta_index, phi_index, 0.0};
   inc_rho = accum.m_delta_rho;
   inc_rho_index = 1;
   // Voting in the RHO array the direction "positive"
   for (/*rho_index*/theta_phi_index[2] = rho_start_index, rho = 0.0 ;; theta_phi_index[2] += inc_rho_index, rho += inc_rho) {
   ...
   inc_rho_index = -1;
   for(theta_phi_index[2]/*rho_index*/ = rho_start_index-1, rho = -inc_rho;;theta_phi_index[2]/*rho_index*/ += inc_rho_index, rho -= inc_rho) {
      if (theta_phi_index[2]/*rho_index*/ < 0) inc_rho_index *= -1;
      if (!accum.process_rho(theta_phi_index/*t, p, rho_index*/)) 
    		  break;
...
void gaussian_vote_2d( accumulatorball_t accum, kernel_t kernel, ArrayList<bin_t> used_bins, 
   ...
   theta_phi_index[3] = rho_start_index;
   // Voting in the PHI array in the direction "inc_phi_index"
   for(theta_phi_index[1]/*phi_index*/ = phi_start_index, phi = phi_start; !voting_ended ;
		   theta_phi_index[1]/*phi_index*/ += inc_phi_index, phi += inc_phi) {    
      theta_phi_index[0] = theta_start_index;
      ...
      for(theta_phi_index[0]/*curr_theta_index*/ = theta_init, theta = 0.0 ;; theta_phi_index[0]/*curr_theta_index*/+=inc_theta_index, theta += inc_theta) {      
         ...
         voted = gaussian_vote_1d(accum, kernel, used_bins, theta_phi_index[0]/*curr_theta_index*/, (int)theta_phi_index[1]/*phi_index*/, (int)theta_phi_index[2]/*rho_index*/, theta, phi);
         if (voted) {
            voting_ended = false;
         } else break;
      }
...
void gaussian_vote_3d(kernel_t kernel, accumulatorball_t accum, ArrayList<bin_t> used_bins){  
      ...
      double[] theta_phi_index = new double[]{kernel.theta_index, kernel.phi_index, 0};
      accum.process_phi(theta_phi_index/*theta_index, phi_index*/);
      gaussian_vote_2d(accum, kernel, used_bins, theta_phi_index[0]/*theta_index*/, (int)theta_phi_index[1]/*phi_index*/, kernel.rho_index, -accum.m_delta_angle, -1);
}
...
void kernel_calculation(octree_t node, accumulatorball_t accum, ArrayList<kernel_t> used_kernels, double max_point_distance){
   ...
   double[] theta_phi_index = new double[]{kernel.theta_index, kernel.phi_index, kernel.rho_index};
   accum.process_limits(theta_phi_index/*kernel.theta_index, kernel.phi_index, kernel.rho_index*/);
   // fill kernel.theta_index, kernel.phi_index, kernel.rho_index
